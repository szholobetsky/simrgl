...
def get_commits_table_name(conn: sqlite3.Connection) -> str:
    """
    Detect the commits table name for a project database.

    Older CodeXplorer databases (e.g. this repo's data/sonar.db) name it
    RAWDATA; newer ones (the per-project DBs under Projects/db/) name it
    COMMITS. Same columns either way - only the table name differs.
    """
    cur = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('COMMITS', 'RAWDATA')"
    )
    ...
    if 'RAWDATA' in tables:
        return 'RAWDATA'
    raise ValueError("Database has neither a COMMITS nor a RAWDATA table")
...
def format_metrics_row(
    ...
    Args:
        ...
        train_source: Index-side source variant name (commit vocabulary)
        query_source: Query-side source variant name (ticket vocabulary)
