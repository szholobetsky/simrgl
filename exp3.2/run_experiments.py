...
class ExperimentRunner:
    ...
    def encode_queries(
        ...
        Args:
            test_set: Test tasks
            source_variant: Key from config.QUERY_SOURCE_VARIANTS

        Returns:
            List of query vectors
        """
        variant_config = config.QUERY_SOURCE_VARIANTS[source_variant]
    ...
    def evaluate_experiment(
        ...
        Args:
            train_source: Index-side source variant key (config.TRAIN_SOURCE_VARIANTS)
                - determines which collection gets queried
            query_source: Query-side source variant key (config.QUERY_SOURCE_VARIANTS)
                - determines how the held-out test tasks get encoded
        ...
        # Build collection name - depends only on the index/train side
        collection_name = config.collection_name(
            train_source, target_variant, window_variant, self.split_strategy,
            model_key=self.model_key
        )

        model_suffix = f"_{self.model_key}" if self.model_key else ""
        experiment_id = f"{train_source}_q{query_source}_{target_variant}_{window_variant}_{self.split_strategy}{model_suffix}"
        ...
        # Encode queries - independent of what built the collection
        query_vectors = self.encode_queries(self.test_set, query_source)
    ...
    def run_single_experiment(
        ...
        Args:
            collection_name: Full collection name (built from train_source by the caller)
            train_source: Index-side source variant key (config.TRAIN_SOURCE_VARIANTS)
            query_source: Query-side source variant key (config.QUERY_SOURCE_VARIANTS)
            target_variant: Target variant key
            window_variant: Window variant key
    ...
    def run_all_experiments(
        ...
        """
        Run all experiment combinations (standalone/debug path - the
        orchestrator in run_comprehensive_experiments.py doesn't call this,
        it calls run_single_experiment() directly per variant)

        Args:
            train_sources: List of index-side source variant keys (default: all)
            query_sources: List of query-side source variant keys (default: all)
        ...
        train_sources = train_sources or list(config.TRAIN_SOURCE_VARIANTS.keys())
        query_sources = query_sources or list(config.QUERY_SOURCE_VARIANTS.keys())
        ...
        logger.info(f"Query Sources: {query_sources}")
            ...
            for query_source in query_sources:
                for target in target_variants:
                    for window in window_variants:
                        result = self.evaluate_experiment(train_source, query_source, target, window)
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help='Index-side source variants to evaluate (default: all)'
    )
    parser.add_argument(
        '--query-sources',
        nargs='+',
        ...
        default=None,
        help='Query-side source variants to evaluate (default: all)'
    )
    parser.add_argument(
        '--targets',
        nargs='+',
