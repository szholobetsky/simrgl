...
# Task-unit criterion: 'ticket' (TASK table, current default), 'commit'
# (synthetic per-commit task built from RAWDATA/COMMITS.MESSAGE - see
# ETLPipeline.load_data_commit_mode()), or 'cross' (exp3.2 only - index
# built from commit messages like 'commit' mode, but queried with the
# ticket-mode test set instead of commit messages - see exp3.2/README.md).
# Override with --task-unit.
TASK_UNIT = 'ticket'
TASK_UNITS = ['ticket', 'commit', 'cross']
...
# Experiment Variants
# RQ2 & RQ3 (exp3.1) / exp3.2 cross-vocabulary grid: the source axis is
# split into two independent dicts instead of one shared SOURCE_VARIANTS.
# TRAIN_SOURCE_VARIANTS builds the index (commit-message vocabulary, used
# by etl_pipeline.py's generate_embeddings()); QUERY_SOURCE_VARIANTS
# encodes the query (ticket vocabulary, used by run_experiments.py's
# encode_queries()). See exp3.2/README.md for the 3x3 grid rationale.

# Index side: commit-message text. 'diff' is the commit's actual changed-
# code content (capped, see ETLPipeline._build_commit_tasks), the
# "extra noisy context" analogue of ticket-mode's 'comments'.
TRAIN_SOURCE_VARIANTS = {
    'title': {
        'name': 'TITLE',
        'description': 'Commit subject line only',
        'fields': ['TITLE']
    },
    'desc': {
        'name': 'TITLE + DESCRIPTION',
        'description': 'Commit subject line and full message',
        'fields': ['TITLE', 'DESCRIPTION']
    },
    'diff': {
        'name': 'TITLE + DESCRIPTION + DIFF',
        'description': 'Commit subject+message plus diff content',
        'fields': ['TITLE', 'DESCRIPTION', 'DIFF']
    }
}

# Query side: Jira ticket text, written before the code change existed.
QUERY_SOURCE_VARIANTS = {
    'title': {
        'name': 'TITLE',
