#!/bin/bash
# Controlled stop for the exp3.2 cross-vocabulary sweep.
#
...
# flock in run_cross_experiment.sh releases automatically on process exit,
# so a clean re-launch is never blocked by this stop.

set -u

PIDS=$(pgrep -f "run_cross_experiment.sh|run_comprehensive_experiments.py")
...
echo "Stopping:"
pgrep -af "run_cross_experiment.sh|run_comprehensive_experiments.py"

kill $PIDS
sleep 2

STILL=$(pgrep -f "run_cross_experiment.sh|run_comprehensive_experiments.py")
if [ -n "$STILL" ]; then
...
if pgrep -f "run_cross_experiment.sh|run_comprehensive_experiments.py" > /dev/null; then
  echo "WARNING: something is still running - check manually with:"
  echo "  pgrep -af run_cross_experiment.sh"
