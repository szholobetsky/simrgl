...
class ComprehensiveExperimentRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            task_unit: 'ticket' or 'commit' (default: config.TASK_UNIT) - scopes results_dir/checkpoint
    ...
    def _run_etl_for_target(
        ...
        """
        Aggregate/upsert a single (source,target,window) variant using
        embeddings already computed by the caller (see
        `_process_source_window`) - embeddings only depend on `source`,
        not `target`/`window`, so computing them once per (source,window)
        and sharing them across targets avoids the ~6x redundant recompute
        the original per-variant implementation had.
        ...
        try:
            ...
            if not target_vectors:
                logger.warning(f"No vectors for {model_key}_{strategy}_{source}_{target}_{window}, skipping")
                return False

            collection_name = config.collection_name(source, target, window, strategy, model_key=model_key)
    ...
    def _process_source_window(
        ...
        """
        Handle every target for one (source, window) pair, computing
        embeddings at most once (skipped entirely if every target already
        has its ETL step checkpointed).
        """
        needs_embedding = any(
            not self.checkpoint.is_etl_completed(model_key, strategy, source, t, window)
        ...
        if needs_embedding:
            try:
                train_tasks_embedded = pipeline.generate_embeddings(train_tasks_windowed, source)
                ...
                error_msg = f"{type(e).__name__}: {str(e)}"
                logger.error(f"[ETL] ✗ Embedding failed for {model_key}_{strategy}_{source}_*_{window}: {error_msg}")
                logger.error(traceback.format_exc())
                for target in self.targets:
                    if not self.checkpoint.is_etl_completed(model_key, strategy, source, target, window):
                        self.checkpoint.mark_etl_failed(model_key, strategy, source, target, window, error_msg)
                        counters['failed'] += 1
                try:
                    clear_gpu_memory()
        ...
        for target in self.targets:
            ...
            etl_success = self._run_etl_for_target(
                model_key, strategy, source, target, window, pipeline, train_merged
            )
            if not etl_success:
                counters['failed'] += 1
            ...
            exp_success = self.run_experiment_variant(model_key, strategy, source, target, window)
            if exp_success:
    ...
    def run_all(self):
        ...
        # Loop order is window -> source -> target (not the more obvious
        # source -> target -> window): embeddings depend only on `source`,
        # and windowing narrows the train set *before* embedding, so this
        # order lets `_process_source_window` compute embeddings once per
        # (source,window) and share them across both `target`s, instead of
        # recomputing per (source,target,window) triple.
        for model_key in self.models:
            for strategy in self.strategies:
                ...
                train_tasks_all, test_tasks = pipeline.create_split(tasks_df)
                ...
                for window in self.windows:
                    ...
                    train_tasks_windowed = pipeline.apply_time_window(train_tasks_all, test_tasks, window_size)

                    log_gpu_memory()

                    for source in self.sources:
                        self._process_source_window(
                            model_key, strategy, source, window,
                            pipeline, train_tasks_windowed, merged_df_base, counters
                        )
