#!/usr/bin/env python3
"""
Three-level progress dashboard for run_full_experiment.sh.

Level 1 - Overall: variant-runs completed across all 19 (project,task_unit) steps.
Level 2 - Current step: variants completed within the (project,task_unit) currently running.
...
LOG_GLOB = 'logs/full_run_*.log'

# Fixed grid in run_full_experiment.sh: 2 models x 2 strategies x 3 sources
# x 2 targets x 3 windows = 72 - same count for every step (ticket and
# commit use different source *names*, but the same count of 3).
VARIANTS_PER_STEP = 2 * 2 * 3 * 2 * 3
TOTAL_STEPS = 19
...
def render():
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("exp3.1 FULL EXPERIMENT DASHBOARD")
    lines.append("=" * 70)
    lines.append("")
