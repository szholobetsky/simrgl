@echo off
echo ========================================
echo RAG ETL Pipeline - Qdrant TEST MODE
echo ========================================
echo.
echo This will create embeddings for TESTING
echo using Qdrant with LAST 1000 tasks only
echo.
echo Parameters:
echo - Backend: Qdrant
...
python etl_pipeline.py --backend qdrant --split_strategy modn --sources desc --targets module file --windows w1000 --model bge-small

echo.
echo ========================================
echo Test ETL Complete!
echo.
echo Collections created in Qdrant:
echo - Module search (~20-30 modules from last 1000 tasks)
echo - File search (~3000-5000 files from last 1000 tasks)
echo.
echo You can now test ragmcp with Qdrant backend
echo.
